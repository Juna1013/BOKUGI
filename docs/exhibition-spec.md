...
作者名入力中のソフトウェアキーボード開閉は表示領域だけの変化として扱い、物理格子とUndo/Redo履歴を変更しません。端末回転など幅を含む実リサイズでは、CPU/GPUの流体状態を縦横比を保つ中央フィットで新しい格子へ移し、旧格子に属するUndo/Redo履歴だけを破棄します。readback、カード生成、Undo/Redo、リサイズは直列化し、処理中の描画入力を一時停止します。
